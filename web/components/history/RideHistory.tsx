...
import { useRouter } from "next/navigation" // Sử dụng cho App Router
...
export function RideHistory() {
    ...
    useEffect(() => {
        if (!userId) return;
        rideAPI.getRidesByRider(userId)
            ...
            {rides.map((ride: any) => (
                ...
                >
                    ...
                    <div className="flex flex-wrap gap-x-8 gap-y-1 text-gray-600 text-sm items-center">
                        ...
                        {ride.driver_id && (
                            <span>Driver: <span className="font-semibold">{ride.driver_id}</span></span>
                        )}
                        <span>Fare: <span className="font-semibold text-orange-600">{formatVND(ride.fare?.total_fare || 0)}</span></span>
                        <span className="flex items-center">
                            Rating:{" "}
                            {ride.rating?.rating
                                ? <>
